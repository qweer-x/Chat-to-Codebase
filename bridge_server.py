...
raise SyncError("没有找到 AUTO_SYNC 协议块。")
...
def backup_existing_file(target: Path, relative_path: str) -> str | None:
...
<!doctype html> <html lang="zh-CN"> <head> <meta charset="utf-8"> <title>AUTO_SYNC Bridge Server</title> <style> body {{ font-family: -apple-system, BlinkMacSystemFont, "Segoe UI", Arial, sans-serif; max-width: 860px; margin: 40px auto; line-height: 1.7; color: #222; }} code {{ background: #f2f2f2; padding: 2px 6px; border-radius: 5px; }} .ok {{ color: #15803d; font-weight: 700; }} .box {{ border: 1px solid #ddd; border-radius: 12px; padding: 16px 20px; background: #fafafa; }} </style> </head> <body> <h1>AUTO_SYNC Bridge Server</h1> <p class="ok">服务正在运行。</p> <div class="box"> <p><b>项目根目录：</b><code>{CONFIG.project_root}</code></p> <p><b>同步接口：</b><code>POST /sync</code></p> <p><b>健康检查：</b><code>GET /health</code></p> <p><b>Git 自动提交：</b><code>{CONFIG.git_enabled}</code></p> <p><b>Git 自动推送：</b><code>{CONFIG.git_push}</code></p> <p><b>备份：</b><code>{CONFIG.backup_enabled}</code></p> <p><b>日志目录：</b><code>{CONFIG.log_dir}</code></p> </div> </body> </html> """ return Response(html, mimetype="text/html; charset=utf-8")
...
raw_blocks = extract_raw_blocks(text)
    ...
    git_result = git_commit_and_push(changed_files) if changed_files else {
